...
- CSVs in `semantic_extraction/` are based on the `direct_extraction/` PDFs and aim to accurately reflect the intended content in the "Content Changes" tables. Where possible, typos are corrected, column headers are normalized, column values are filled down (where unambiguous), and inline headings are represented in a new "Section Description" column. Multi-line values that apply to several rows are also resolved.