...
class TestBaselineCSVContent:
    ...
    def test_all_schedules_present(self, records: list[dict[str, Any]]) -> None:
        """Schedule codes with line-based instructions are present in the baseline CSV.

        Column-format schedules (20, 22A, 22B, 28, 61A, 61B, 62, 74A, 74B, 74C, 74D, 74E, 81)
        legitimately produce zero records and are excluded from this check.
        """
        column_format_schedules: frozenset[str] = frozenset(
            {"20", "22A", "22B", "28", "61A", "61B", "62", "74A", "74B", "74C", "74D", "74E", "81"}
        )
        ...
        column_format_schedules: frozenset[str] = frozenset(
            {
                ...
                "74A",  # Debt Burden — Column X structure
                "74B",  # Foreign Currency Debt — Column X structure
                "74C",  # Contingent Liabilities — Column X structure
